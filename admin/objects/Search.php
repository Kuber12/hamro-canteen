...
class Search {
  ...
  public function orderBySort($columnName, $query,$sortby) {
    $query = '%' . $query . '%';
    $sql = "SELECT fullName,phone,payment,status,orderID,orderedTime,gtotal FROM orders inner join users on orders.userID = users.userID WHERE $columnName LIKE ? ORDER BY $columnName $sortby";
    if (!empty($orderby)) {
      $sql .= " ";
    }
